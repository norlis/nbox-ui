...
export function EditableRow({
    ...
    const getDisplayValue = () => {
        if (!isRevealable(entry)) return entry.value;
        if (isRevealed) {
    ...
    return (
        <tr
            className={`border-b border-slate-700 transition-all duration-200 ${
                shouldEdit ? "bg-yellow-500/5 border-yellow-500/20" :
            ...
            >
                {shouldEdit ? (
                    ...
                    <div
                        ...
                        role="button"
                        tabIndex={0}
                        aria-label={`Copy reference of ${entry.key}`}
                        onKeyDown={(e) => { if (e.key === "Enter" || e.key === " ") copyToClipboard(`{{ ${entryId} }}`, `${entryId}`) }}
                    >
            ...
            <td className="p-4 w-2/4">
                {shouldEdit ? (
                    <div className="flex items-center space-x-2">
                        <Input
                            ref={valueInputRef}
                            type={isRevealable(entry) && !isRevealed ? "password" : "text"}
                            value={currentValue}
                    ...
                    {shouldEdit ? (
                        <>
                            <Button
                                ...
                                size="sm"
                                onClick={handleSave}
                                className="h-8 w-8 p-0 text-green-400 hover:text-green-300 transition-colors"
                            ...
                            <Button
                                ...
                                className="h-8 w-8 p-0 text-red-400 hover:text-red-300 transition-colors"
